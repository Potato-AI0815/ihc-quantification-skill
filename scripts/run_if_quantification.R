...
for (idx in seq_along(image_ids)) {
  ...
  all_image_qc[[length(all_image_qc) + 1L]] <- data.table(
    ...
    median_cell_propagation_radius = seg_res$metrics$median_cell_propagation_radius,
    mean_cell_propagation_radius = seg_res$metrics$mean_cell_propagation_radius,
    nonzero_cell_propagation_fraction = seg_res$metrics$nonzero_cell_propagation_fraction,
