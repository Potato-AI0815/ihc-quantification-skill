...
3. **Multi-channel IF (v2.3.0-rc1)**: Supports TIFF and ImageJ-compatible hyperstacks; OME-TIFF metadata workflows remain under validation. Tested intensity preservation covers 8/16/32-bit float and 12-bit detector-range values stored in a 16-bit container; packed native 12-bit TIFF is not formally validated. The workflow also provides background correction (Rolling Ball / Top-hat), 4-compartment quantification (GLOBAL, NUCLEUS, CYTOPLASM, EXTRACELLULAR), single-cell N/C ratios, dual-channel colocalization (Pearson $r$, Manders $M_1/M_2$; colocalization does not establish molecular binding), and a validated synthetic puncta counting workflow (DoG).
