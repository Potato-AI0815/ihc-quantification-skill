...
> - **Current release gate**: The validation hotfix and exact `v2.3.0-alpha.2` main-commit Ubuntu/Windows CI pass ([Actions run 32555682952](https://github.com/Potato-AI0815/ihc-quantification-skill/actions/runs/32555682952)); this remains an alpha prerelease with the warnings documented below.
...
3. **Multi-channel IF (v2.3.0-alpha.2)**: Supports TIFF and ImageJ-compatible hyperstacks; OME-TIFF metadata workflows remain under validation. Tested intensity preservation covers 8/16/32-bit float and 12-bit detector-range values stored in a 16-bit container; packed native 12-bit TIFF is not formally validated. The workflow also provides background correction (Rolling Ball / Top-hat), 4-compartment quantification (GLOBAL, NUCLEUS, CYTOPLASM, EXTRACELLULAR), single-cell N/C ratios, dual-channel colocalization (Pearson $r$, Manders $M_1/M_2$; colocalization does not establish molecular binding), and a validated synthetic puncta counting workflow (DoG).
