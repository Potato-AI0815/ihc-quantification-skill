...
**Release Milestone**: `v2.3.2` — external real-data validation evidence
...
- Wortmannin: plate-wide positive-control reference (150 nM, column 12) median N/C = 4.7762 vs negative controls (columns 1–2) = 0.7026; effect = +4.0735; Z-prime (descriptive) = 0.6635.
- LY294002: high-dose reference (80 µM, column 1 — the maximum dose of the official LY294002 series) median N/C = 4.8914 vs negative controls (column 2) = 0.7315; maximum-dose reference effect = +4.1599; Z-prime (descriptive) = 0.5348.
...
All 4 external benchmarks evaluate to **`PASS`** or **`PASS_WITH_WARNINGS`** under frozen parameter baselines, with every summary artifact numerically aligned to the underlying result CSVs by the consistency gate. The repository satisfies the criteria for the current release milestone **`v2.3.2`**; the validation evidence itself originates from the immutable **`v2.3.0-rc3`** baseline.
