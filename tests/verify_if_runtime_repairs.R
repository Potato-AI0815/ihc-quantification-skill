...
# When the synthetic IF fixture has already been generated by the smoke test,
# exercise the defensive cell-boundary contract as a regression check.  The
# conditional keeps this focused test runnable in isolation before fixtures are
# built.
if_fixture <- file.path(root, "tests", "synthetic_if_fixture")
if_image <- file.path(if_fixture, "images", "IF_S01_CTRL_F1.tif")
if_manifest <- file.path(if_fixture, "manifest.csv")
if (file.exists(if_image) && file.exists(if_manifest)) {
  if_rows <- data.table::fread(if_manifest)[image_id == "IF_S01_CTRL_F1"]
  if_read <- read_if_image(if_image, if_rows, z_mode = "max_projection")
  if_nuc <- preprocess_if_channel(
    if_read$channels[["DAPI"]], method = "top_hat", radius = 25,
    bit_depth = if_read$image_meta$bit_depth
  )$corrected_mat
  if_seg <- segment_if_image(
    nuclear_mat = if_nuc,
    cyto_ref_mat = if_read$channels[["Alexa568_Tubulin"]]
  )
  union_components <- max(EBImage::bwlabel(if_seg$cell_labels > 0), na.rm = TRUE)
  if (if_seg$n_cells < 2L || max(if_seg$cell_labels, na.rm = TRUE) != if_seg$n_cells) {
    stop("Cell-label regression failed: one or more nuclei lost their cell label.")
  }
  if (union_components < 2L ||
      if_seg$metrics$effective_cell_propagation_radius > if_seg$metrics$max_cytoplasm_expansion_radius) {
    stop("Cell-boundary regression failed: propagated territories merged or exceeded the cap.")
  }
  cat(sprintf(
    "PASS: IF cell-boundary regression (nuclei=%d, union_components=%d, effective_radius=%d, gap=%d).\n",
    if_seg$n_cells, union_components,
    if_seg$metrics$effective_cell_propagation_radius,
    if_seg$metrics$cytoplasm_boundary_gap_px
  ))
}
